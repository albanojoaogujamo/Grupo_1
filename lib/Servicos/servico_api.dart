...
class ServicoApi {
  ...
  // ------------------------------------------------
  // PERGUNTAS — se backend falhar, usa as locais
  // ------------------------------------------------
  Future<List<Pergunta>> buscarPerguntas() async {
    try {
      final resposta = await http.get(
        Uri.parse(Constantes.urlPerguntas),
        headers: _cabecalhos,
      );
      if (resposta.statusCode == 200) {
        final List<dynamic> dados = jsonDecode(resposta.body);
        return dados.map((p) => Pergunta.fromJson(p)).toList();
      }
      return _perguntasLocais();
    } catch (e) {
      // sem ligação — usa perguntas no código
      return _perguntasLocais();
    }
  }

  // ------------------------------------------------
  // PERGUNTAS LOCAIS (sem necessidade de servidor)
  // ------------------------------------------------
  List<Pergunta> _perguntasLocais() {
    return [
  ...
  // PONTUAÇÕES
  // ------------------------------------------------
  Future<void> guardarPontuacao(Pontuacao pontuacao) async {
    try {
      await http.post(
        Uri.parse(Constantes.urlPontuacoes),
        headers: _cabecalhos,
        body: jsonEncode({
          'idUtilizador': pontuacao.idUtilizador,
          'nomeUtilizador': pontuacao.nomeUtilizador,
          'pontos': pontuacao.pontos,
          'data': pontuacao.data.toIso8601String(),
        }),
      );
    } catch (e) {
      // sem ligação — ignora silenciosamente
    }
  }

  Future<List<Pontuacao>> buscarClassificacao() async {
    try {
      final resposta = await http.get(
        Uri.parse(Constantes.urlPontuacoes),
        headers: _cabecalhos,
      ...
      if (resposta.statusCode == 200) {
        final List<dynamic> dados = jsonDecode(resposta.body);
        return dados.map((p) => Pontuacao.fromJson(p)).toList();
      }
      return [];
    } catch (e) {
      return [];
